...
def draw(ax,
    ...
    # Draw logo
    logo.draw(ax, logo_style=logo_style)


def show_motifs(motif_dict, motif_height, motif_width, num_motifs_across, \
                show_name=True, return_fig=False, **kwargs):
    num_motifs = len(motif_dict)
    num_cols = num_motifs_across
    num_rows = np.ceil(num_motifs / num_motifs_across)
    fig_height = motif_height * num_rows
    fig_width = motif_width * num_cols
    fig = plt.figure(figsize=[fig_width, fig_height])
    motif_names = motif_dict.keys()
    motif_names.sort()
    for n, motif_name in enumerate(motif_names):
        prob_df = motif_dict[motif_name]
        ax = fig.add_subplot(num_rows, num_cols, n + 1)
        draw(ax, prob_df=prob_df, **kwargs)
        if show_name:
...
def logo_on_matrix(ax, energy_df, relative_scale=1, relative_spacing=.1,
    ...
    draw(ax, effect_df=energy_df, logo_type='information', find_beta=True)
    L = len(energy_df)
    ax.imshow(energy_df.values.T, aspect='auto',
